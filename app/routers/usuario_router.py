...
from app.schemas import compra_schemas, usuario_schemas
...
def novo_usuario(usuario: usuario_schemas.UsuarioNovo):
    resultado = usuario_services.novo_usuario(usuario.dict())
...
@router.post('/usuario-compra')
def usuario_compra(compra: compra_schemas.Compra):
    resultado = usuario_services.efetuar_compra(compra)
    return {'resultado': resultado}
